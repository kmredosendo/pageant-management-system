...
// This assumes you have a 'Talent' criteria or event. Adjust as needed.
export async function GET() {
  // Find the main criteria with the identifier 'best-in-talent' for the active event
  ...
  const mainCriteria = await prisma.criteria.findFirst({
    where: { eventId: event.id, parentId: null, identifier: "best-in-talent" },
  });
